...
def simulateMotion(type = 0, nbr_targets = 8, nbr_trials = 1, outPath = "UserErrorSimResults/", metaPath = "UserErrorSimResults/meta_data.json", simSteps = 250):
    ...
    metaDict['parameters'] = {'x': start_x, 'y': start_y, 'z': start_z, 'points': points, 'time_scalar': tscalar, 'simulation_steps': simSteps}
    ...
    for qq in range(nbr_trials):
        for i in range(nbr_targets):
            ...
            solver.set_optim_max_iterations(simSteps)

            predictSolution = study.solve()
            successState = predictSolution.success()
            solutionUnsealed = predictSolution.unseal()
            filename = outPath + "torque_solutions" + fileLoc + "/solution" + str(target) +ascii_lowercase[qq] + '.sto'
            ...
            filename = outPath + "path_solutions" + fileLoc + "/traj" + str(target) + ascii_lowercase[qq] + '.sto'
            f = open(filename, "a")
            f.truncate(0)
            ...
            f.close()
            metaDict["trial_" + str(qq) + "_target_" + str(i)] = {"trial":qq, "target": i, "r":r,"theta":theta,"success":successState,"target_x":X,"target_y":Y,"target_z":Z}
            
    meta_f = open(metaPath, "w")
    meta_f.write(json.dumps(metaDict, indent = 4))
    meta_f.close()
